 ...
 */
  ...
  var BigNumber,
    ...
    POWS_TEN = [1, 10, 100, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20,1e21,1e22,1e23,1e24,1e25,1e26,1e27,1e28,1e29,1e30],
    SQRT_BASE = 1e7,

    // EDITABLE
    // The limit on the value of DECIMAL_PLACES, TO_EXP_NEG, TO_EXP_POS, MIN_EXP, MAX_EXP, and
    // the arguments to toExponential, toFixed, toFormat, and toPrecision.
    MAX = 1e14;                                   // 0 to MAX_INT32
...
//b, d, f, h are BigNumber objects
	...
	if (a==e){
		realMag=BigNumber.sum(b,f);
	} else {
//		realMag=Math.abs(b-f);
		realMag=(b.minus(f)).abs();
}

	if (c==g){
		imMag=BigNumber.sum(d,h);
	} else {
		imMag=(d.minus(h)).abs();
}

	if (a==e && BigNumber.sum(b,f) != 0){
	realSign=a;	
	}
	...
	if (BigNumber.sum(b,f)==0){
	realSign=0;	
}

	if (c==g && BigNumber.sum(d,h) != 0){
	imSign=c;	
	}
...
function fmpModulusEuclidean(a,b,c,d,e){
//e is the quantity of signs in the multipolar system > 1
	let euclideanInterval=(BigNumber.sum(b.exponentiatedBy(2),d.exponentiatedBy(2))).sqrt();
...
//IF F==0, IF F==1...
	...
	if (f==0){
		...
		let outputSign=0;
		let outputMag=new BigNumber(1);
		let outputSign2=0;
		let outputMag2=new BigNumber(0);
...
function factorial(n){
    ...
    if (nBN.isEqualTo(0))
        return one;
    return nBN.multipliedBy(factorial(n - 1));


}
	one = new BigNumber(1);
	zero = new BigNumber(0);
	let consolid=consolidate(0,one,0,zero,a,b,c,d);

	for (let i=0; i<n-1;i++){
	
	denominator = new BigNumber(factorial(i+2));
	one = new BigNumber(1);
	zero = new BigNumber(0);
	let prequotient = one.dividedBy(denominator);

console.log(i + " " + denominator);	
console.log(i + " " + prequotient);
//	let quotient=(fmpComplexMultiply(...fmpNaturalExponent(a,b,c,d,e,(i+2)),0,(1/(factorial(i+2))),0,0,e));

	let quotient=(fmpComplexMultiply(...fmpNaturalExponent(a,b,c,d,e,(i+2)),0,prequotient,0,zero,e));
...
zero = new BigNumber(0);
pi = new BigNumber('3.14159265358979323846264338327950288419716939937510582097494459230781640628620');
//There is no limit to the number of digits of a string input (notice the apostrophes). Math.PI output is to 15 decimal places.


//BigNumber('3.');
//pi to 400 digits
...
console.log(pi);
//theta = (pi.dividedBy(20)).times(i);
//console.log(" " +  " " + i, fmpExpPowerSeries(0,zero,2,theta,6,600));
//console.log(" " +  " " + i, fmpExpPowerSeries(6,1,0,0,6,17));
//}
        
//console.log(" " +  "Modulus Natural:" + fmpModulusNatural(4,2,0,2,6,17));             
//console.log(" " +  "Power Series:" + fmpExpPowerSeries(0,0,2,Math.PI,6,200));     
//console.log(" " +  "Power Series:" + fmpExpPowerSeries(0,0,2,Math.PI*3,6,17));  
//console.log(" " +  "Power Series:" + fmpExpPowerSeries(0,0,2,Math.PI*3,6,170));  
//console.log(" " +  "Power Series:" + fmpExpPowerSeries(0,0,2,Math.PI*6,6,170));  
//console.log(" " +  "Power Series:" + fmpExpPowerSeries(0,0,2,Math.PI*10,6,170));  
        //as theta increases in magnitude, greater n is required to "reign in" the initial quotient addends
//No failure at argument ~18 and n of 300. (320 decimal places)
//Yes failure at argument ~36 and n of 300 (320 decimal places).
//Yes failure at argument ~36 and n of 600 (320 decimal places).
//Starting to slip at argument ~36 and n of 750 or 850 (320 decimal places), but not >1 magnitude.
//same at argument ~36 and n of 850 (420 decimal places)
//increase pi to 400 digits--too slow
//


//console.log(fmpForwardDivision(0,25,0,0,6,5,0,0,6));
//console.log("MI"+fmpMultiplicativeInverse(8,.5,4,.5,6));
//console.log(fmpComplexMultiply(6,.71,4,.71,6,.70422,6,.70422,6));
//console.log(fmpComplexMultiply(0,1,0,1,6,1,0,1,6));
//console.log("NaturalExponent"+fmpNaturalExponent(0,2,0,3,6,2));
//console.log(fmpArgument(8,1,10,1,6));
//console.log("Sqrt"+fmpSqrt(4,.4,6,.8,6));
//console.log(fmpNaturalExponent(2, 1.5723027555148466, 10, 2.544039299028138,6,2));
//console.log(fmpNaturalExponent(2, 5.5723027555148466, 10, 6.544039299028138,6,2));
//console.log(fmpNaturalExponent(2, .71, 10, 2.71, 6,2));
//console.log(fmpNaturalExponent(2, 3, 6, 5, 10,2));
//console.log(fmpNaturalExponent(2, 3, 16, 5, 10,2));
//console.log(fmpNaturalExponent(2, 5, 6, 3, 10,2));
//console.log(fmpNaturalExponent(2,0.8044958641907105,4,0.4972057878785785, 6,2));
console.log(testNum = new BigNumber(pi.multipliedBy(22)));
//console.log(xxxx = 5);
//console.log(testNum);
//console.log(consolidate(0,testNum,2,testNum,0,testNum,0,testNum,2));
//console.log(fmpMultiply(0,testNum,0,testNum,2));
//console.log(fmpComplexMultiply(0,testNum,0,testNum,0,testNum,0,testNum,2));
//console.log(fmpNaturalExponent(0,testNum,0,testNum,2,2));
//console.log(fmpModulusNatural(0,testNum,0,testNum,2));
//console.log(fmpModulusEuclidean(0,testNum,0,testNum,2));
//console.log(zero = new BigNumber(0));

//console.log(fmpExpPowerSeries(0,zero,2,testNum,6,600));
console.log(fmpExpPowerSeries(0,zero,2,testNum,6,9000));
//no timeout if n==600, decimal places ==1.5k, |theta|==12pi --good approximation
//no timeout if n==600, decimal places ==1.5k, |theta|==18pi --good approximation
//no timeout if n==600, decimal places ==1.5k, |theta|==24pi --VERY BAD approximation
//[if |theta|==24pi, decimal place 1.5k @ n==631]
//no timeout if n==800, decimal places ==1.7k, |theta|==24pi --VERY BAD approximation
//no timeout if n==800, decimal places ==2k, |theta|==24pi --VERY BAD approximation
//no timeout if n==800, decimal places ==2k, |theta|==20pi --OK approximation
 //[if |theta|==20pi, decimal place 2k @ n==806]
//TIMEOUT if n==900, decimal places ==2.3k, |theta|==20pi [without web workers]
//w/webworkers if n==900, decimal places ==2.3k, |theta|==20pi --OK approximation
 //[if |theta|==20pi, decimal place 2.3k @ n==908]
//w/webworkers if n==1200, decimal places ==3.2k, |theta|==20pi --OK approximation
//w/webworkers if n==1500, decimal places ==3.7k, |theta|==20pi --OK approximation
 //[if |theta|==20pi, decimal place 3.7k @ n==1366]
 //w/webworkers if n==1500, decimal places ==4.2k, |theta|==20pi --OK approximation
  //[if |theta|==20pi, decimal place 5k @ n==1773]
 //w/webworkers if n==2000, decimal places ==5.3k, |theta|==20pi --OK approximation
   //[if |theta|==24pi, decimal place 5.3k @ n==1866]
      //[if |theta|==22pi, decimal place 5.5k @ n==1927]
     //[if |theta|==22pi, decimal place 7k @ n==2376]
 //w/webworkers if n==6000, decimal places ==20000, |theta|==22pi --VERY BAD approximation
     //[if |theta|==22pi, decimal place 20k @ n==5980]
 //w/webworkers if n==9000, decimal places ==40000, |theta|==22pi --?? approximation
